...
def save_idsw_to_json(switch_data, video_name):
    """Save identity switch data to a JSON file."""
    
    output_dir = os.getcwd() + '/output/tracker_to_eval/idsw_train'
    os.makedirs(output_dir, exist_ok=True)
    file_path = os.path.join(output_dir, f"{video_name}.json")
...
class CLEAR(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(
            ...
            if np.any(is_idsw):
                for i in np.where(is_idsw)[0]:
                    gt_id = matched_gt_ids[i]  # Ground truth object ID
                    prev_id = prev_matched_tracker_ids[i]  # Previous tracker ID
                    current_id = matched_tracker_ids[i]  # Current tracker ID

                    # Append switch details into the list
                    id_switches.append({
                        "frame": t+1, # visualize from frame 1
                        "gt_id": int(gt_id),  
                        "previous_tracker_id": int(prev_id) + 1, # predict from id 1
                        "current_tracker_id": int(current_id) + 1 # predict from id 1
                    })
                    # print("idsw", t+1, int(prev_id) + 1, '->', int(current_id) + 1)
        ...
        save_idsw_to_json(id_switches, video_name=data['seq'])
