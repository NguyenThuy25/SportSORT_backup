...
def embedding_distance_tracks(track1s, track2s, metric='cosine'):
    cost_matrix = np.zeros((len(track1s), len(track2s)), dtype=np.float)
    if cost_matrix.size == 0:
        return cost_matrix
    track1_features = np.asarray([track.smooth_feat for track in track1s], dtype=np.float)
    track2_features = np.asarray([track.smooth_feat for track in track2s], dtype=np.float)
    cost_matrix = np.maximum(0.0, cdist(track1_features, track2_features, metric))  # / 2.0  # Nomalized features
    return cost_matrix
