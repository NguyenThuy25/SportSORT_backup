...
class PlayerClassification:
    ...
    def get_crop(self, image, bboxes, type):
        crops = []
        for bbox in bboxes:
            if type == 'xywh':
                x1, y1, w, h = bbox[:4]
...
def save_data_pred_npy_det(pose_model, cache_folder, img_folder, team_names):
...
def save_data_pred(pose_model, cache_folder, img_folder, team_names):
    ...
    for i, video_name in enumerate(video_list):
       ...
       # Load detection data from .txt file
        video_detection = []
        with open(detection_path, 'r') as file:
            for line in file:
                # Parse line into list of floats
                detection = list(map(float, line.strip().split(',')))
                video_detection.append(detection)
        video_detection = np.array(video_detection, dtype=object)
        # video_length = video_detection.shape[0]
        frame_ids = np.unique(video_detection[:, 0]).astype(int)
        seqinfo_path = os.path.join(img_folder, video_name, "seqinfo.ini")
       
        video_length = int(open(seqinfo_path, "r").readlines()[4].split("=")[1])
        
        video_team_data = {}
        for team_name, team_folder in team_folders.items():
            video_team_data[team_name] = np.empty((video_length,), dtype=object)
            

        # scale = min(1440/1280, 800/720)
        pc = PlayerClassification()
        for i, frame_id in enumerate(frame_ids):
            frame_detection = video_detection[video_detection[:, 0] == frame_id]
            frame_detection = frame_detection[:, 2:6]  # Select bounding box columns (x, y, width, height)

            frame = cv2.imread(os.path.join(img_folder, video_name, "img1", f"{str(frame_id).zfill(6)}.jpg"))
            pose_results = inference_topdown(pose_model, frame, frame_detection, 'xyxy')
...
if __name__ == "__main__":
    ...
    pose_config = 'rtmpose/config/rtmpose-l_8xb256-420e_coco-256x192.py'
    pose_checkpoint = 'rtmpose/ckpt/rtmpose-l_simcc-aic-coco_pt-aic-coco_420e-256x192-f016ffe0_20230126.pth'
    device = 'cuda:0'
    ...
    for cache_folder, img_folder in zip(cache_folders, img_folders):
        save_data_pred_npy_det(pose_model, cache_folder, img_folder, team_names)
    ################################

    # result_folder = "/mnt/banana/student/thuyntt/data/SoccerNet/tracking/visualize_team"
    # cache_folder = "/mnt/banana/student/thuyntt/Deep-EIoU/cache_soccer/test"
    # img_folder = "/mnt/banana/student/thuyntt/data/SoccerNet/tracking/test"
    # team_name = "team_full"
