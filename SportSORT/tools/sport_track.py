...
def main():
    ...
    seq_path =  os.path.join(data_path,'fix_detection/')
    ...
    for seq in seqs:
        ...
        if not os.path.exists(os.path.join(data_path,'fix_detection/','{}.npy'.format(seq))):
            continue

        detections = np.load(os.path.join(data_path,'fix_detection/','{}.npy'.format(seq)),allow_pickle=True)
        
        embeddings = np.load(os.path.join(data_path,'fix_embedding/','{}.npy'.format(seq)),allow_pickle=True)
            
        sct_output_path = os.path.join(data_path,'SCT','{}.txt'.format(seq))
        
        detections = [np.array(det) for det in detections]
        # SCT tracking
        # detections: num_frame x num_detections x 7
