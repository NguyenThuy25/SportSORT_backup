...
def image_track(detections, embeddings, team_embeddings, sct_output_path, args):
    ...
    for frame_id,det in enumerate(detections,1):
        ...
        team_embs = team_embeddings[frame_id-1]
        ...
        if det is not None:
            ...
            # online_targets = tracker.update_backup(det, embs)  
            trackerTimer.toc()
...
def main():
    ...
    for seq in seqs:
        ...
        team_embeddings = np.load(os.path.join(data_path,'team_full/','{}.npy'.format(seq)),allow_pickle=True)
            
        sct_output_path = os.path.join(data_path,'SCT','{}.txt'.format(seq))
        
        # SCT tracking
        image_track(detections, embeddings, team_embeddings, sct_output_path, args)
