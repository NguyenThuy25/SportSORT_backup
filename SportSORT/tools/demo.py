...
from SportSORT.tracker.SportSORT import Deep_EIoU
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                for i, (x1, y1, x2, y2, *rest) in enumerate(det):
                    ...
                    if x2 > x1 and y2 > y1:  # Make sure the box has valid dimensions
                        # Crop the image if the box is valid
                        cropped_img = frame[y1:y2, x1:x2]
                        cropped_imgs.append(cropped_img)
                    else:
                        # Set the detection at index i to zero
                        invalid_indices.append(i)
                        # det[i] = np.zeros_like(det[i])
