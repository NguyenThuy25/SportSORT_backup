...
#     # file_path = os.path.join(args.root_path,'SCT')
#     # txt_path = args.txt_path
#     save_path = args.save_path
#     if args.save_path is None:
#         save_path = Path(args.txt_path).parent / 'interpolation'
#         mkdir_if_missing(save_path)

#     # 

#     for scene in os.listdir(args.txt_path):
#         scene = scene.replace('.txt','')
#         # input_path = os.path.join(file_path,'{}.txt'.format(scene))
#         input_path = os.path.join(args.txt_path,'{}.txt'.format(scene))
#         output_path = os.path.join(save_path,'{}.txt'.format(scene))
#         dti(input_path, output_path, n_min=args.n_min, n_dti=args.n_dti, distance_thres=args.distance_thres)
if __name__ == '__main__':
    
    args = make_parser().parse_args()
    # file_path = os.path.join(args.root_path,'SCT')
    file_path = '/mnt/banana/student/thuyntt/Deep-EIoU/evaluation/TrackEval/data/thuy_res/result_test/46'

    # mkdir_if_missing(os.path.join(args.root_path,'interpolation'))
    mkdir_if_missing(os.path.join('/mnt/banana/student/thuyntt/Deep-EIoU/evaluation/TrackEval/data/thuy_res','interpolation_test'))
        
    for scene in os.listdir(file_path):
        scene = scene.replace('.txt','')
        input_path = os.path.join(file_path,'{}.txt'.format(scene))
        # output_path = os.path.join(args.root_path,'interpolation','{}.txt'.format(scene))
        output_path = os.path.join('/mnt/banana/student/thuyntt/Deep-EIoU/evaluation/TrackEval/data/thuy_res','interpolation_test','{}.txt'.format(scene))
        dti(input_path, output_path, n_min=args.n_min, n_dti=args.n_dti, distance_thres=args.distance_thres)